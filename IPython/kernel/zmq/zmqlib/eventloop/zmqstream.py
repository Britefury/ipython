    ...
    class ZMQStream(object):
        ...
        * **stop_on_send():**
        ...
        def __init__(self, socket, io_loop=None):
            ...
            # shortcircuit some socket methods (note we are running on Jython here...)
            self.bind = self.socket.bind
            self.bind_to_random_port = self.socket.bindToRandomPort
            ...
            self.setsockopt = err
            self.getsockopt = err
            self.setsockopt_string = err
            self.getsockopt_string = err
            self.setsockopt_unicode = err
            self.getsockopt_unicode = err
        ...
        def flush(self, flag=ZMQ_POLLIN|ZMQ_POLLOUT, limit=None):
            ...
            def update_flag():
                ...
                return flag & ZMQ_POLLIN | (self.sending() and flag & ZMQ_POLLOUT)
            flag = update_flag()
            if not flag:
                # nothing to do
                return 0
            ...
            while events and (not limit or count < limit):
                s,event = events[0]
                if event & ZMQ_POLLIN: # receiving
                    self._handle_recv()
                    count += 1
                    if self.socket is None:
                        # break if socket was closed during callback
                        break
                if event & ZMQ_POLLOUT and self.sending():
                    self._handle_send()
                    count += 1
                    if self.socket is None:
                        # break if socket was closed during callback
        ...
        def _handle_recv(self):
            ...
            if self._flushed:
                return
            try:
                msg = self.socket.recv_multipart(ZMQ_NOBLOCK, copy=self._recv_copy)
            except ZMQError as e:
                if e.errno == ZMQ_EAGAIN:
        ...
        def _handle_send(self):
            ...
            try:
                status = self.socket.sendMultipart(msg, **kwargs)
            except ZMQError as e:
